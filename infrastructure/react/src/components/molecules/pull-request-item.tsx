  ...
  return (
    ...
    >
      ...
      <box width={widths.Title}>
        <text fg={theme.foreground}>{truncateText(item.title, widths.Title)}</text>
      </box>
      ...
      <box width={widths.Target}>
        <text fg={theme.foreground}>{truncateText(item.taget, widths.Target)}</text>
      </box>
      ...
      <box width={widths.MR}>
        {item.review.myStatus.kind === "as_author" && (
          <text
            fg={theme[getReviewStatusConfigMe(item.review.myStatus).color as keyof typeof theme]}
          >
      ...
      <box width={widths.Review} flexDirection="row" gap={1}>
        <ReviewStatus statusOverall={item.review.overallStatus} statusMe={item.review.myStatus} />
      </box>
      ...
      {/* Context menu */}
      {isContextMenuOpen && <ContextMenu id={String(item.id.number)} options={menuOptions} />}
    </box>
  );
}
