...
export function PullRequestManager() {
  ...
  useAutoScroll(scrollRef, height, itemFocusStore.current?.index, filterablePrs.length);
  ...
  useKeyboard((key) => {
    ...
    if (isAction(key.name, "up") || isAction(key.name, "down")) {
      itemFocusStore.next(matchKey(key.name) as YDirectionsActions, filterablePrs);
    }
    ...
    if (isAction(key.name, "copy")) {
      ...
      if (!branch) return toastActions.error("Could not copy the branch name : branch not found");
  ...
  return (
    <box
      ...
      borderStyle="rounded"
      borderColor={
        tabFocusStore.current === Tabs.PULL_REQUESTS ? theme.focusedBorder : theme.border
      }
    >
      {(searchMode || filter !== "") && (
        <box
