...
export const keyMap = Object.entries(keyDefintion).reduce((previous, current) => {
...
export function isAction(key: string, action: SupportedActions | SupportedActions[]): boolean {
  const foundAction = matchKey(key);
  if (Array.isArray(action)) return action.includes(foundAction as SupportedActions);
