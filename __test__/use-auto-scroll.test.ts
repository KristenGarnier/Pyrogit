...
describe("calculateScrollPosition", () => {
  ...
  it("should handle custom options with different itemHeight and overhead", () => {
    expect(calculateScrollPosition(20, 2, 10, { itemHeight: 2, overhead: 2 })).toBe(0); // lowBound=4, 2<4, 0
    expect(calculateScrollPosition(20, 5, 10, { itemHeight: 2, overhead: 2 })).toBe(1); // 4<5, 5-4=1
    expect(calculateScrollPosition(20, 7, 10, { itemHeight: 2, overhead: 2 })).toBe(10); // 7>6, 10
  });
});
...
describe("useAutoScroll", () => {
  ...
  it("should update scroll position when currentIndex changes", () => {
    ...
    const { rerender } = renderHook(
      ({ currentIndex }) => useAutoScroll(mockScrollRef as any, 20, currentIndex, 10),
